...
const addNumberToInstructions = (number: number) => {
  if (instructions) {
    instructions.textContent += number.toString();
  }
};

const addSymbolToInstructions = (symbol: string) => {
...
Object.keys(numbers).forEach((key: string) => {
  const buttonKey = key as keyof Numbers;
...
Object.keys(symbols).forEach((key: string) => {
  const buttonKey = key as keyof Symbols;
...
equal.addEventListener("click", (event) => {
  ...
  if (instructionsValue !== "") {
    const theresNoSimbol = !instructionsValue
      ?.split("")
      .some(
        (item) => item === "/" || item === "*" || item === "+" || item === "-"
      );
    if (theresNoSimbol) {
      ...
      if (secondValue === "") {
        ...
        const operator = instructionsValue
          ?.split("")
          .find(
            (operator) =>
              operator === "/" ||
